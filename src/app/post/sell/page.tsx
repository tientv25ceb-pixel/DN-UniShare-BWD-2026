      ...
      <main className="min-h-screen flex flex-col">
        ...
        <div className="flex-grow flex items-center justify-center p-4">
          <div className="card p-10 rounded-2xl max-w-md w-full text-center animate-in">
            ...
            <p className="text-sm text-[var(--muted-foreground)] mb-8">Món đồ của bạn đang được rao bán. Chúc bạn sớm tìm được người mua.</p>
            <div className="flex flex-col gap-3">
              <button onClick={() => router.push('/items')} className="btn-primary justify-center w-full py-3">Xem danh sách</button>
              <button onClick={() => { setSubmitted(false); setForm({ title: '', description: '', price: '', category: '', condition: '', location: '', image: '' }); }} className="btn-outline justify-center w-full py-3">Đăng bán món khác</button>
            </div>
          </div>
        </div>
        <Footer />
      ...
      <div className="flex-grow pt-28 pb-16">
        <div className="container mx-auto px-4 md:px-6 max-w-2xl">
          <div className="text-center mb-10">
            ...
            <h1 className="text-3xl font-bold text-[var(--foreground)] mb-2">Bán đồ</h1>
            <p className="text-sm text-[var(--muted-foreground)]">Thanh lý đồ cũ — giá sinh viên thân thiện</p>
          </div>

          <form onSubmit={handleSubmit} className="card p-6 md:p-8 rounded-2xl border border-amber-500/10">
            <div className="mb-6 p-4 rounded-xl bg-amber-500/5 border border-amber-500/10">
              <label className="block text-xs font-semibold text-[var(--muted-foreground)] uppercase tracking-wider mb-2">Giá bán *</label>
              <div className="flex items-center gap-2">
                <Tag size={20} className="text-amber-400 shrink-0" />
                <input type="number" required min={0} placeholder="50000" className="w-full text-2xl font-bold bg-transparent border-b-2 border-amber-500/30 focus:border-amber-400 outline-none py-1 text-[var(--foreground)] placeholder:text-gray-600" value={form.price} onChange={e => setForm(p => ({ ...p, price: e.target.value }))} />
            ...
            <div className="space-y-5">
              <div>
                ...
                <input type="text" required placeholder="VD: Tai nghe Sony WH-1000XM4" className="input-field" value={form.title} onChange={e => setForm(p => ({ ...p, title: e.target.value }))} />
              </div>
              <div>
                <label className="block text-xs font-semibold text-[var(--muted-foreground)] uppercase tracking-wider mb-1.5">Mô tả *</label>
                <textarea required rows={4} placeholder="Mô tả chi tiết: tình trạng, lý do bán, thời gian sử dụng..." className="input-field resize-none" value={form.description} onChange={e => setForm(p => ({ ...p, description: e.target.value }))} />
              </div>
              ...
              <div>
                ...
                <label className="w-full h-[120px] rounded-xl border-2 border-dashed border-[var(--border)] bg-[var(--card)] flex flex-col items-center justify-center text-[var(--muted-foreground)] cursor-pointer hover:border-amber-400 hover:bg-amber-500/5 transition-colors overflow-hidden">
            ...
            <div className="mt-6 pt-6 border-t border-[var(--border)] space-y-3">
              <label className="block text-xs font-semibold text-[var(--muted-foreground)] uppercase tracking-wider flex items-center gap-1.5"><MapPin size={14} className="text-amber-400" /> Địa điểm gặp mặt *</label>
    ...
    <main className="min-h-screen flex flex-col">
      ...
      <div className="flex-grow flex items-center justify-center p-4">
        <div className="auth-gate">
          ...
          <p className="text-sm text-[var(--muted-foreground)] mb-6">Bạn cần đăng nhập để bán đồ.</p>
