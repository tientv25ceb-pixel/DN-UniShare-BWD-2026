      ...
      <main className="min-h-screen flex flex-col">
        ...
        <div className="flex-grow flex items-center justify-center p-4">
          <div className="text-center">
            ...
            <button onClick={() => router.push('/')} className="dn-btn-primary">Về trang chủ</button>
          </div>
        </div>
        <Footer />
      </main>
  ...
  if (saved) {
    return (
      <main className="min-h-screen flex flex-col">
        <Header />
        <div className="flex-grow flex items-center justify-center p-4">
          <div className="dn-card p-10 rounded-2xl max-w-md w-full text-center">
            <div className="h-20 w-20 rounded-full bg-[var(--dn-accent-soft)] flex items-center justify-center mx-auto mb-6 text-[var(--dn-accent)]"><CheckCircle2 size={40} /></div>
            <h2 className="text-2xl font-bold mb-3">Đã cập nhật!</h2>
            <p className="text-sm text-[var(--dn-text-secondary)] mb-8">Món đồ của bạn đã được cập nhật thành công.</p>
            <div className="flex flex-col gap-3">
              <button onClick={() => router.push(`/detail/${id}`)} className="dn-btn-primary justify-center w-full py-3">Xem bài đăng</button>
              <button onClick={() => router.push('/profile')} className="dn-btn-outline justify-center w-full py-3">Về trang cá nhân</button>
            </div>
          </div>
        </div>
        <Footer />
    ...
    <main className="min-h-screen flex flex-col">
      ...
      <div className="flex-grow pt-28 pb-16 relative overflow-hidden">
        <div className="container mx-auto px-4 md:px-6 max-w-2xl">
          <button onClick={() => router.back()} className="flex items-center gap-1.5 text-sm text-[var(--dn-text-secondary)] hover:text-[var(--dn-text-primary)] font-medium mb-6 transition-colors">
            ← Quay lại
          </button>

          <div className="text-center mb-10">
            <h1 className="text-3xl font-bold mb-2">Chỉnh sửa bài đăng</h1>
            <p className="text-sm text-[var(--dn-text-secondary)]">Cập nhật thông tin món đồ của bạn</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div>
              ...
              <input type="text" required placeholder="VD: Giáo trình Giải tích 1" className="dn-input" value={form.title} onChange={e => setForm(p => ({ ...p, title: e.target.value }))} />
            </div>

            <div>
              <label className="block text-xs font-semibold text-[var(--dn-text-secondary)] uppercase tracking-wider mb-1.5">Mô tả *</label>
              <textarea required rows={4} placeholder="Mô tả chi tiết món đồ..." className="dn-input resize-none" value={form.description} onChange={e => setForm(p => ({ ...p, description: e.target.value }))} />
            </div>

            {isSale && (
              <div>
                <label className="block text-xs font-semibold text-[var(--dn-text-secondary)] uppercase tracking-wider mb-1.5">Giá bán *</label>
                <div className="flex items-center gap-2">
                  <input type="number" required min={0} placeholder="50000" className="w-full dn-input" value={form.price} onChange={e => setForm(p => ({ ...p, price: e.target.value }))} />
            ...
            {isLostOrFound && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 p-4 rounded-xl bg-red-500/5 border border-red-500/10">
                <div>
                  <label className="block text-xs font-semibold text-[var(--dn-text-secondary)] uppercase tracking-wider mb-1.5">Thời gian mất/nhặt</label>
                  <input type="date" className="dn-input" value={form.lostDate} onChange={e => setForm(p => ({ ...p, lostDate: e.target.value }))} />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-[var(--dn-text-secondary)] uppercase tracking-wider mb-1.5">Hậu tạ / Thưởng</label>
                  <input type="text" placeholder="VD: 50.000đ" className="dn-input" value={form.reward} onChange={e => setForm(p => ({ ...p, reward: e.target.value }))} />
                </div>
                <div className="md:col-span-2">
                  <label className="block text-xs font-semibold text-[var(--dn-text-secondary)] uppercase tracking-wider mb-1.5">SĐT liên hệ</label>
                  <input type="text" placeholder="VD: 0912345678" className="dn-input" value={form.contactPhone} onChange={e => setForm(p => ({ ...p, contactPhone: e.target.value }))} />
                </div>
              </div>
            )}
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <select required className="dn-input" value={form.category} onChange={e => setForm(p => ({ ...p, category: e.target.value }))}>
              ...
              <div>
                ...
                <select required className="dn-input" value={form.condition} onChange={e => setForm(p => ({ ...p, condition: e.target.value }))}>
            ...
            <div>
              ...
              <label className="w-full h-[120px] rounded-xl border-2 border-dashed border-[var(--dn-border-strong)] bg-[var(--dn-surface-muted)] flex flex-col items-center justify-center text-[var(--dn-text-secondary)] cursor-pointer hover:border-[var(--primary)] hover:bg-[var(--primary)]/5 transition-colors overflow-hidden">
            ...
            <div className="space-y-3 pt-2 border-t border-[var(--dn-border-strong)]">
              <label className="block text-xs font-semibold text-[var(--dn-text-secondary)] uppercase tracking-wider flex items-center gap-1.5"><MapPin size={14} /> Địa điểm hẹn gặp *</label>
